...
DataDir = '/Volumes/svalbard/mitgcm/sp/BUMP/B3/run/allmnc/'
run = 'B3'
...
for ti, tt in enumerate(b6['T'].values):
	...
	# v
	h = ax[0].pcolormesh(v.Yp1.values/1000, grid.Z.values, np.squeeze(v.isel(T=ti).values), cmap='RdBu_r', vmin=-0.1, vmax=0.1)
	...
	# w
	h = ax[1].pcolormesh(w.Y.values/1000, grid.Z.values, np.squeeze(w.isel(T=ti).values), cmap='RdBu_r', vmin=-0.012, vmax=0.012)
